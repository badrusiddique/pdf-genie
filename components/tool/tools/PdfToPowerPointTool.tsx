...
export function PdfToPowerPointTool({ tool }: { tool: Tool }) {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/v1/process/pdf-to-powerpoint', { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    return (
      <div className="max-w-xl mx-auto px-6 py-12">
        <ToolResult
          ...
          fileName={file ? `${file.name.replace(/\.pdf$/i, '')}.pptx` : 'presentation.pptx'}
          onReset={handleReset}
        />
      </div>
    )
  ...
  const sidebar = (
    <div className="space-y-4">
      <div className="flex gap-2.5 p-3 rounded-lg text-xs leading-relaxed"
        ...
        <Info className="w-4 h-4 shrink-0 mt-0.5" style={{ color: '#F87171' }} />
        <span>Each PDF page becomes one slide. Text content extracted as slides — original design is not preserved.</span>
      </div>
      {error && (
  ...
  const action = (
    <button onClick={handleConvert} disabled={!file || status === 'processing'}
      ...
      style={{
        background: file ? 'linear-gradient(135deg, #EF4444 0%, #DC2626 100%)' : 'rgba(255,255,255,0.05)',
        color: '#ffffff', boxShadow: file ? '0 4px 20px rgba(239,68,68,0.35)' : 'none',
      }}>
      <span>Convert to PowerPoint</span><span className="text-lg">→</span>
    </button>
  )

  return (
    <ToolLayout sidebar={sidebar} action={action}
      sidebarHeader={<h2 className="font-display text-base font-semibold" style={{ color: '#F1F5F9' }}>PDF to PowerPoint</h2>}>
      {!file ? (
        <div role="button" tabIndex={0} aria-label="Upload PDF to convert to PowerPoint"
          className="flex flex-col items-center justify-center gap-4 rounded-2xl cursor-pointer transition-all duration-200"
          style={{ minHeight: '360px', background: over ? 'rgba(239,68,68,0.06)' : 'rgba(255,255,255,0.02)', border: `2px dashed ${over ? 'rgba(239,68,68,0.5)' : 'rgba(255,255,255,0.10)'}` }}
          onDragOver={e => { e.preventDefault(); setOver(true) }}
          onDragLeave={() => setOver(false)}
          onDrop={e => { e.preventDefault(); setOver(false); const f = e.dataTransfer.files[0]; if (f) setFile(f) }}
          onClick={() => document.getElementById('pdf-ppt-input')?.click()}
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') document.getElementById('pdf-ppt-input')?.click() }}>
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
            style={{ background: over ? 'rgba(239,68,68,0.15)' : 'rgba(255,255,255,0.04)' }}>
            <Presentation className="w-8 h-8" style={{ color: over ? '#F87171' : '#475569' }} />
          </div>
          ...
          <input id="pdf-ppt-input" type="file" accept="application/pdf" className="sr-only"
