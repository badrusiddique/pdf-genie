...
export function PdfToWordTool({ tool }: { tool: Tool }) {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/v1/process/pdf-to-word', { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    return (
      <div className="max-w-xl mx-auto px-6 py-12">
        <ToolResult
          ...
          fileName={file ? `${file.name.replace(/\.pdf$/i, '')}.docx` : 'document.docx'}
          onReset={handleReset}
        />
      </div>
    )
  ...
  const sidebar = (
    <div className="space-y-4">
      <div className="flex gap-2.5 p-3 rounded-lg text-xs leading-relaxed"
        ...
        <span>Text content preserved. Original layout (columns, images) may not be retained in the DOCX output.</span>
      </div>
      {error && (
  ...
  const action = (
    <button onClick={handleConvert} disabled={!file || status === 'processing'}
      ...
      <span>Convert to Word</span><span className="text-lg">→</span>
    </button>
  )

  return (
    <ToolLayout sidebar={sidebar} action={action}
      sidebarHeader={<h2 className="font-display text-base font-semibold" style={{ color: '#F1F5F9' }}>PDF to Word</h2>}>
      {!file ? (
        <div role="button" tabIndex={0} aria-label="Upload PDF to convert to Word"
          ...
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') document.getElementById('pdf-word-input')?.click() }}>
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
            style={{ background: over ? 'rgba(124,58,237,0.15)' : 'rgba(255,255,255,0.04)' }}>
            <FileText className="w-8 h-8" style={{ color: over ? '#A78BFA' : '#475569' }} />
          </div>
          ...
          <input id="pdf-word-input" type="file" accept="application/pdf" className="sr-only"
